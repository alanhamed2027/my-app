...
const EditUserModal = ({ isOpen, onClose, onUpdated, user }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!name.trim() || !username.trim()) {
      setError('تکایە ناو و ناوی بەکارهێنەر پڕبکەرەوە');
      return;
    }
    ...
    try {
      ...
      setError(err.response?.data?.error || 'هەڵەیەک ڕوویدا لە کاتی گۆڕینی زانیارییەکان');
    } finally {
      setLoading(false);
    }
  };
        ...
        >
          ...
          <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-700 px-6 py-5 bg-slate-50/50 dark:bg-slate-900/50">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-lg font-black text-slate-800 dark:text-slate-100">گۆڕانکاری لە کارمەند</h3>
            </div>
            <button
              onClick={onClose}
            ...
            <form id="edit-user-form" onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 px-4 py-2.5 text-sm font-medium text-slate-800 dark:text-slate-100 focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20"
                >
              ...
              <div className="pt-4 border-t border-slate-200 dark:border-slate-700">
                <p className="text-xs font-bold text-slate-400 mb-3">ئەگەر ناتەوێت پاسۆردەکەی بگۆڕیت، ئەم بەشە بەتاڵ جێبهێڵە:</p>
                <Input
                  label="وشەی تێپەڕی نوێ (پاسۆرد)"
                  type="password"
                  value={password}
                  onChange={(e) => setPassword(e.target.value)}
                  placeholder="پاسۆردی نوێ (بە ئارەزوومەندانە)"
                  dir="ltr"
                />
              </div>
